...
// Update feed back functional componenet
const UpdateFeedBacks = () => {
  ...
  //  reset fields by selecting single data by its id
  const onReset = async () => {
    const result = await dispatch(singleFeedBack(id));
    reset(result.payload);
  };
  //  submit data
  const onSubmit = (data) => {
    // pass the data to formData
        ...
        <div class="mx-auto container max-w-2xl md:w-3/4 shadow-md">
          <div class="bg-gray-100 p-4 border-t-2 bg-opacity-5 border-indigo-400 rounded-t">
            <div class="max-w-sm mx-auto md:w-full md:mx-0">
              ...
              <Typography variant="h6">Update Passenger Feedback</Typography>
              </div>
            </div>
          </div>
